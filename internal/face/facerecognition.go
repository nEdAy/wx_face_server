...
import (
	"errors"
	"github.com/nEdAy/face-golang-sdk"
...
// GetFaceRecognitionFaceCount 获取人脸数 Face++
func GetFaceRecognitionFaceCount(picPath string) (int, string, error) {
	// 上传脸部信息到face++
...
// AddFaceRecognitionTokenToFaceSet 添加到FaceSet Face++
func AddFaceRecognitionTokenToFaceSet(faceToken string) (string, error) {
	faceSDK, err := sdk.NewFaceSDK(config.CFG.APIKey, config.CFG.APISecret, config.CFG.Debug)
	if err != nil {
		return "", err
	}
...
// RemoveFaceRecognitionFace 删除FaceSet中人脸
func RemoveFaceRecognitionFace(faceToken, facesetToken string) error {
	// 删除face++
	faceSDK, err := sdk.NewFaceSDK(config.CFG.APIKey, config.CFG.APISecret, config.CFG.Debug)
	if err != nil {
		return err
...
// SearchFaceRecognitionFaceToken 搜索人脸
func SearchFaceRecognitionFaceToken(picPath string) (string, error) {
	// 验证用户脸部信息
	faceSDK, err := sdk.NewFaceSDK(config.CFG.APIKey, config.CFG.APISecret, config.CFG.Debug)
	if err != nil {
		return "", err
